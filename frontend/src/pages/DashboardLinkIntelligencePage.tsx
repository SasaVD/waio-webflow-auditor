...
export default function DashboardLinkIntelligencePage() {
  ...
  if (!tipr) {
    const showWaiting = enrichment.status === 'polling' || enrichment.status === 'timed_out';
    return (
      <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-sm text-text-secondary mt-1">
            PageRank, CheiRank, and link equity recommendations across your site.
          </p>
        </motion.div>
        ...
        {showWaiting ? (
          ...
          >
            ...
            <p className="text-sm font-semibold text-text mb-1">
              {enrichment.status === 'timed_out'
                ...
                : 'Link intelligence is being generated'}
            </p>
            <p className="text-xs text-text-muted max-w-md mx-auto">
              {enrichment.status === 'timed_out'
                ? 'This can take up to 20 minutes for larger sites. TIPR analysis will appear automatically when ready.'
          ...
          >
            ...
            <p className="text-sm font-semibold text-text mb-1">
              Link Intelligence unavailable for this site
            </p>
            <p className="text-xs text-text-muted max-w-md mx-auto">
              {enrichment.progress || 'The site blocks automated crawlers (Cloudflare or similar), so no link data was returned. TIPR analysis requires a successful multi-page crawl.'}
            </p>
          </motion.div>
