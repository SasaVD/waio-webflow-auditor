...
export function generateMarkdown(report: Record<string, any>): string {
  ...
  if (report.categories) {
    ...
    for (const [key, cat] of Object.entries(report.categories)) {
      ...
      lines.push(
        `| ${pillarLabels[key] || key} | ${catObj.score ?? '-'} | ${catObj.label ?? '-'} |`
      );
    }
    lines.push('');
  }
  ...
  if (report.categories) {
    ...
    for (const [key, cat] of Object.entries(report.categories)) {
      ...
      lines.push(`### ${pillarLabels[key] || key}`);
      lines.push('');
  ...
  lines.push('---');
  lines.push('*Generated by WAIO Audit Tool — Veza Digital*');
  lines.push('');
...
export function downloadMarkdown(report: Record<string, any>): void {
  ...
  a.href = url;
  a.download = `WAIO-Audit-${domain}-${date}.md`;
